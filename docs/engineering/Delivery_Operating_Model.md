...
The GitHub Wiki is human-facing orientation and synthesis only.  It is never canonical for backlog, Kanban, roadmap, product decisions, acceptance criteria, or architecture specification - those remain owned by the documents in the table above and in `BACKLOG.md`'s "Authority and Scope" section.  Synchronize the Wiki at the cadence in the table; do not update it after every PR, and do not let it accumulate detail that duplicates a faster-moving canonical source.
