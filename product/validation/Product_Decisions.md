...
| [DEC-030](#dec-030) | PV-029; DS-01 evaluation | Web Editorial Direction target configuration: `gpt-5.6-terra` and Variant D instructions |
...
|---|---|
| Decision ID | DEC-030 |
| Validation Reference | PV-029; DS-01 evaluation (`docs/product/version2/Editorial_Direction_Quality_Investigation.md`) |
| Date Approved | 2026-08-11 |
| Decision | Adopt the Web Product's Editorial Direction target configuration: model `gpt-5.6-terra` (replacing `gpt-4o-mini`), and the "Variant D" improved Editorial Direction system-prompt instructions evaluated in DS-01 (`IMPROVED_SYSTEM_PROMPT` in `web/server/eval/editorial-direction-eval.mjs`, replacing the current production system prompt in `web/server/src/openai/editorialDirection.ts`).  This decision is scoped narrowly to model selection and prompt instructions.  It explicitly does NOT authorize any change to: the Structured Outputs schema (`JSON_SCHEMA` / `editorialDirectionSchema`), the persisted `EditorialDirection` shape, source retrieval/processing (`retrieveSource`), project workflow state, or Approve/Reject behavior - all remain unchanged by this decision. |
| Reason | PV-029 recorded that hosted Web Editorial Direction was technically functional but materially weaker editorially than the locked GPT baseline, without prejudging a cause.  DS-01's difference analysis and two controlled evaluation passes against materially different real sources (cybersecurity research; NASA/AI-policy) produced the same D > C > B > A quality ordering both times, with a model effect (A -> C) that repeated strongly holding the prompt constant, a prompt effect (A -> B) that repeated directionally though with variable magnitude, and no material contradiction between the two passes.  The Repository Author judged this evidence sufficient to approve Variant D (`gpt-5.6-terra` plus the improved instructions) as the target configuration, without further evaluation calls. |
| Affected Product Areas | Web Product Editorial Direction generation (`web/server/src/openai/`). |
| Implementation Status | Implemented and hosted-verified.  The bounded delivery this decision authorized (model selection and Editorial Direction instructions only, with schema, persisted shape, source-processing, workflow state, Approve/Reject, authentication, Render infrastructure, the locked GPT, and GPT Knowledge unchanged) was completed with application-path regression tests and one bounded real local verification, merged to `develop` as PR #130 (commit `afb07eb`), and confirmed by post-merge hosted acceptance through the real Web Product path at `https://studio.ramrattan.com` on 2026-08-11 (see `docs/product/version2/Editorial_Direction_Quality_Investigation.md` Section 16).  Web Walking Skeleton 02 remains out of scope and unauthorized by this decision. |
| Repository References | `product/validation/Product_Validation_Log.md` PV-029; `docs/product/version2/Editorial_Direction_Quality_Investigation.md`; `web/server/eval/editorial-direction-eval.mjs`; `web/server/src/openai/editorialDirection.ts` and `client.ts` (implemented); `web/server/tests/editorialDirection.model.test.ts`; `render.yaml`; `BACKLOG.md` BL-001, BL-002. |
