...
| Implementation Status | Approved; NOT YET IMPLEMENTED.  Implementation is authorized as a separate, bounded delivery, scoped to exactly: (1) centralized Editorial Direction model selection and (2) the approved Editorial Direction instructions.  It must not change schema, the persisted `EditorialDirection` shape, source-processing behavior, project workflow state, Approve/Reject behavior, authentication, Render infrastructure, the locked GPT, or GPT Knowledge, and must include application-path regression tests plus one bounded real hosted/local verification through the actual Web Product path.  Web Walking Skeleton 02 remains out of scope and unauthorized by this decision. |
| Repository References | `product/validation/Product_Validation_Log.md` PV-029; `docs/product/version2/Editorial_Direction_Quality_Investigation.md`; `web/server/eval/editorial-direction-eval.mjs`; `web/server/src/openai/editorialDirection.ts` and `client.ts` (implementation targets, not yet modified); `BACKLOG.md` BL-001, BL-002. |
